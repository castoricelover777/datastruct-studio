...
关键帧到源码行的映射覆盖率：**83 / 86 = 97%**（其余是没有代码片段的纯文字步骤）。
...
对照陈越《数据结构》课程大纲，**已完成 / 待补** 一目了然（应用内点左下角「模块覆盖清单」也能看到实时版本）：

| 章 | 节 | 状态 |
| --- | --- | --- |
| **01 引论** | 01-01 算法复杂度 | ✅ 4 个模块 · 4 段动画 |
| | 01-02 最大子列和问题（四算法对比 O(n³)→O(n)） | ✅ 5 个模块 · 5 段动画 |
| **02 线性结构** | 02-01 线性表（顺序表） | ✅ 6 个模块 · 6 段动画 |
| | **02-02 链表**（单链表 13 + 双链表 15，含对比视图） | ✅ 26 个模块 · 26 段动画 |
| | 02-03 堆栈 | ✅ 5 个模块 · 5 段动画 |
| | 02-04 队列（循环队列） | ✅ 5 个模块 · 5 段动画 |
| | 02-05 应用实例（中缀转后缀 + 求值） | ✅ 4 个模块 · 4 段动画 |
| **03 树** | 二叉树 / BST / AVL / 堆 / 哈夫曼 / 并查集 | ⏳ 架构已预留 |
| **04 图** | 表示 / 遍历 / 最短路径 / 最小生成树 / 拓扑 | ⏳ 架构已预留 |
| **05 排序** | 冒泡插入希尔 / 堆归并 / 快排 / 基数 | ⏳ 架构已预留 |
| **06 散列** | 散列函数 / 开放地址 / 分离链接 | ⏳ 架构已预留 |

**已完成 7 / 25 节：55 个练习模块、55 段动画、63 个模块（含 7 个拼装视图）。**

> 架构上六章全部就位（章色带、三层树、数据 schema 都通了），补内容只需在
> `resources/reference/<节>/` 下写标注源码 + 在 `resources/animations/` 写场景，然后重新构建。
...
切档只增删注释行，**代码行逐字节相同**——这一条由自检强制保证（1263 项检查里有专门一项）。
...
**当前：2645 项，0 失败**（含"每个练习模块都必须配动画"这一条）。
...
- **v2.0（本次）** 架构升级 + 三层树 + 动画播放器 + 代码联动 + 链表大模块 + 01/02 章内容
- **v2.1** 补齐 02 剩余节 + 树 / 图 / 排序 / 散列
